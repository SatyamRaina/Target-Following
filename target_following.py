...
class TargetFollowerWithObstacleAvoidance:
    ...
    def move_robot(self, detections):
        ...
        if abs(z - self.target_distance_threshold) > 0.05:
            self.stop_robot()
            rospy.sleep(1)
            if z > self.target_distance_threshold:
                self.goal_distance(0.1, 0.2)  # Move forward
            else:
                self.goal_distance(-0.1, -0.2)  # Move backward
        elif abs(x) > 0.05:
            self.stop_robot()
            rospy.sleep(1)
            if x > 0:
                self.goal_angle(-math.pi / 4, 1.0)  # Turn left
            else:
                self.goal_angle(math.pi / 4, 1.0)  # Turn right
        else:
            cmd_msg = Twist2DStamped()
            cmd_msg.header.stamp = rospy.Time.now()
    ...
    def goal_distance(self, distance, linear_speed):
        ...
        target_ticks = (distance / (2 * math.pi * self.wheel_radius)) * 100
        while abs(init_tick - self.tick_count) < target_ticks:
            cmd_msg = Twist2DStamped()
            cmd_msg.header.stamp = rospy.Time.now()
            cmd_msg.v = linear_speed  # Straight line velocity
            cmd_msg.omega = 0.0
            self.cmd_vel_pub.publish(cmd_msg)
            rospy.loginfo("Moving forward/backward!")
        self.stop_robot()
